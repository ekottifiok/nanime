---
title: Installation
description: Learn how to install and configure nanime in your Nuxt project.
navigation:
  icon: i-lucide-download
seo:
  title: Installation - nanime
  description: Step-by-step guide to installing nanime via CLI or npm, and configuring it in nuxt.config.ts.
---

# Installation

Get started with `nanime` in your Nuxt project in just a few steps.

## Automatic Installation

The easiest way to install the module is via the `nuxi` CLI:

::code-group
```bash [npx]
npx nuxi module add nanime
```
```bash [pnpm]
pnpm dlx nuxi module add nanime
```
```bash [bun]
bun x nuxi module add nanime
```
::

This command will automatically install the package and add it to your `nuxt.config.ts`.

## Manual Installation

If you prefer to install manually:

::steps
### Install the package

Install `nanime` using your package manager of choice:

::code-group
```bash [npm]
npm install nanime
```
```bash [pnpm]
pnpm add nanime
```
```bash [yarn]
yarn add nanime
```
```bash [bun]
bun add nanime
```
::

### Configure Nuxt

Add `nanime` to the `modules` section of your `nuxt.config.ts`:

```ts [nuxt.config.ts]
export default defineNuxtConfig({
  modules: ['nanime']
})
```
::
